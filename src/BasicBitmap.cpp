...
const IUINT32 _pixel_scale_4[16] = { 0, 16, 32, 49, 65, 82, 98, 115,
    139, 156, 172, 189, 205, 222, 238, 255 };

/* lookup table for scaling 5 bit colors up to 8 bits */
const IUINT32 _pixel_scale_5[32] = { 0, 8, 16, 24, 32, 41, 49, 57,
...
#define RGBA_FROM_X8R8G8B8(c, r, g, b, a) \
    do {                                  \
        _pixel_disasm_X888(c, r, g, b);   \
        (a) = 255;                        \
    } while (0)
#define RGBA_FROM_P8R8G8B8(c, r, g, b, a)    \
    do {                                     \
        _pixel_from_P8R8G8B8(c, r, g, b, a); \
    } while (0)
#define RGBA_FROM_A8B8G8R8(c, r, g, b, a) _pixel_disasm_8888(c, a, b, g, r)
#define RGBA_FROM_R8G8B8(c, r, g, b, a) \
    do {                                \
        _pixel_disasm_888(c, r, g, b);  \
        (a) = 255;                      \
    } while (0)
#define RGBA_FROM_R5G6B5(c, r, g, b, a) \
    do {                                \
        _pixel_disasm_565(c, r, g, b);  \
        (a) = 255;                      \
    } while (0)
#define RGBA_FROM_X1R5G5B5(c, r, g, b, a) \
    do {                                  \
        _pixel_disasm_X555(c, r, g, b);   \
        (a) = 255;                        \
    } while (0)
...
#define _pixel_RGBA_to_P8R8G8B8(r, g, b, a) \
    ((((a)) << 24) | ((((r)*_pixel_norm(a)) >> 8) << 16) | ((((g)*_pixel_norm(a)) >> 8) << 8) | ((((b)*_pixel_norm(a)) >> 8) << 0))

#define _pixel_from_P8R8G8B8(c, r, g, b, a)            \
    do {                                               \
        IUINT32 __SA = ((c) >> 24);                    \
        IUINT32 __FA = (__SA);                         \
        (a) = __SA;                                    \
...
#define BLEND_STATIC(sr, sg, sb, sa, dr, dg, db, da)                   \
    do {                                                               \
...
#define BLEND_NORMAL(sr, sg, sb, sa, dr, dg, db, da)                   \
    do {                                                               \
        IINT32 SA = _pixel_norm(sa);                                   \
...
#define BLEND_NORMAL_FAST(sr, sg, sb, sa, dr, dg, db, da)              \
    do {                                                               \
...
#define BLEND_NORMAL_FLAT(sr, sg, sb, sa, dr, dg, db, da) \
    do {                                                  \
        (dr) = ((((sr) - (dr)) * (sa)) / 255) + (dr);     \
        (dg) = ((((sg) - (dg)) * (sa)) / 255) + (dg);     \
        (db) = ((((sb) - (db)) * (sa)) / 255) + (db);     \
...
/* additive blend */
#define BLEND_ADDITIVE(sr, sg, sb, sa, dr, dg, db, da) \
    do {                                               \
...
#define BLEND_SRCOVER(sr, sg, sb, sa, dr, dg, db, da) \
    do {                                              \
        IUINT32 SA = 255 - (sa);                      \
        (dr) = (dr)*SA;                               \
        (dg) = (dg)*SA;                               \
...
#define BLEND_PARGB(color_dst, color_src)                        \
    do {                                                         \
...
#define BLEND_PARGB_COVER(color_dst, color_src, coverage) \
    do {                                                  \
...
//---------------------------------------------------------------------
BasicBitmap::BasicBitmap(int width, int height, PixelFmt fmt, void* mem,
    long pitch)
{
    _bits = NULL;
    _lines = NULL;
...
// Destructor
//---------------------------------------------------------------------
BasicBitmap::~BasicBitmap() { Destroy(); }
...
// for bpp = 15 is A1R5G5B5 and for bpp = 8 is G8.
//---------------------------------------------------------------------
int BasicBitmap::Initialize(int width, int height, PixelFmt fmt, void* mem,
    long pitch)
{
    int bpp = 0;
...
// Constructor: create new bitmap
//---------------------------------------------------------------------
void BasicBitmap::Clear(IUINT32 color) { Fill(0, 0, _w, _h, color); }

//---------------------------------------------------------------------
// blit normal
//---------------------------------------------------------------------
int BasicBitmap::BlitNormal(int bpp, void* dbits, long dpitch, int dx,
    const void* sbits, long spitch, int sx, int w,
    int h, IUINT32 mask, int flip)
{
...
// blit mask
//---------------------------------------------------------------------
int BasicBitmap::BlitMask(int bpp, void* dbits, long dpitch, int dx,
...
//---------------------------------------------------------------------
int BasicBitmap::ClipRect(const int* clipdst, const int* clipsrc, int* x,
    int* y, int* rectsrc, int mode)
{
...
//---------------------------------------------------------------------
int BasicBitmap::ClipScale(const BasicRect* clipdst, const BasicRect* clipsrc,
    BasicRect* bound_dst, BasicRect* bound_src,
    int mode)
{
    ...
    if (dl >= dcl && dt >= dct && dr <= dcr && db <= dcb && sl >= scl && st >= sct && sr <= scr && sb <= scb)
        return 0;
...
//---------------------------------------------------------------------
int BasicBitmap::Blit(int x, int y, const BasicBitmap* src, int sx, int sy,
    int w, int h, int mode)
{
    ...
    switch (_pixelsize) {
    case 1:
        hook = opacity ? PixelBlitNormal1 : PixelBlitMask1;
        break;
    case 2:
        hook = opacity ? PixelBlitNormal2 : PixelBlitMask2;
        break;
    case 3:
        hook = opacity ? PixelBlitNormal3 : PixelBlitMask3;
        break;
    case 4:
        hook = opacity ? PixelBlitNormal4 : PixelBlitMask4;
        break;
    }

    if (hook != NULL) {
        int retval = hook(dbits, _pitch, x, sbits, src->_pitch, sx, w, h, mask, flip);
        if (retval == 0)
            return 0;
    }

    if (opacity) {
        BlitNormal(_bpp, dbits, _pitch, x, sbits, src->_pitch, sx, w, h, mask,
            flip);
    } else {
        BlitMask(_bpp, dbits, _pitch, x, sbits, src->_pitch, sx, w, h, mask,
            flip);
    }
...
void BasicBitmap::SetDriver(int bpp, PixelBlit proc, bool transparent)
{
    ...
    case 16:
        if (transparent)
            ...
            PixelBlitNormal2 = proc;
        break;
    case 24:
        if (transparent)
            PixelBlitMask3 = proc;
        else
            PixelBlitNormal3 = proc;
        break;
    case 32:
        if (transparent)
            PixelBlitMask4 = proc;
        else
            PixelBlitNormal4 = proc;
        break;
    }
}
...
{
    if ((unsigned int)x >= (unsigned int)_w)
        return;
    if ((unsigned int)y >= (unsigned int)_h)
        return;
    IUINT8* bits = Line(y);
    switch (_pixelsize) {
    case 1:
        _pixel_store(8, bits, x, color);
        break;
    case 2:
        _pixel_store(16, bits, x, color);
        break;
    case 3:
        _pixel_store(24, bits, x, color);
        break;
    case 4:
        _pixel_store(32, bits, x, color);
        break;
    }
}
...
// fetch pixel
//---------------------------------------------------------------------
void BasicBitmap::Fetch(PixelFmt fmt, const void* bits, int x, int w,
    IUINT32* buffer)
{
    IUINT32 r, g, b, a;
    switch (fmt) {
    ...
    case A8R8G8B8: {
        internal_memcpy(buffer, (const IUINT32*)bits + x,
            w * sizeof(IUINT32));
    } break;
    case A8B8G8R8: {
    ...
    case X8R8G8B8: {
        ...
        ILINS_LOOP_DOUBLE({ *buffer++ = *src++ | 0xff000000; },
            {
...
// store pixel
//---------------------------------------------------------------------
void BasicBitmap::Store(PixelFmt fmt, void* bits, int x, int w,
    const IUINT32* buffer)
{
    IUINT32 r, g, b, a;
    switch (fmt) {
    case G8: {
    ...
    case A8B8G8R8: {
        ...
        ILINS_LOOP_DOUBLE(
            {
                *dst++ = (buffer[0] & 0xff00ff00) | ((buffer[0] & 0xff0000) >> 16) | ((buffer[0] & 0xff) << 16);
                buffer++;
            },
            {
                *dst++ = (buffer[0] & 0xff00ff00) | ((buffer[0] & 0xff0000) >> 16) | ((buffer[0] & 0xff) << 16);
...
#define PIXEL_SPAN_DRAW_PROC_N(fmt, bpp, nbytes, mode)                   \
    \
static int pixel_span_draw_proc_##fmt##_0(void* bits, int offset, int w, \
        const IUINT32* card)                                             \
    \
{                                                                   \
    ...
    \
static int pixel_span_draw_proc_##fmt##_1(void* bits, int offset, int w, \
        const IUINT32* card)                                             \
    \
{                                                                   \
        unsigned char* dst = ((unsigned char*)bits) + offset * nbytes;   \
    ...
    \
static int pixel_span_draw_proc_##fmt##_2(void* bits, int offset, int w, \
        const IUINT32* card)                                             \
    \
{                                                                   \
        unsigned char* dst = ((unsigned char*)bits) + offset * nbytes;   \
...
BasicBitmap::PixelDraw BasicBitmap::GetDriver(BasicBitmap::PixelFmt fmt,
    int mode, bool builtin)
{
    static int inited = 0;
    PixelDraw proc = NULL;
    ...
    case A8R8G8B8:
        if (mode == 0)
            proc = pixel_span_draw_proc_A8R8G8B8_0;
        else if (mode == 1)
            proc = pixel_span_draw_proc_A8R8G8B8_1;
        else
            proc = pixel_span_draw_proc_A8R8G8B8_2;
        break;
    case A8B8G8R8:
        if (mode == 0)
            proc = pixel_span_draw_proc_A8B8G8R8_0;
        else if (mode == 1)
            proc = pixel_span_draw_proc_A8B8G8R8_1;
        else
            proc = pixel_span_draw_proc_A8B8G8R8_2;
        break;
    case X8R8G8B8:
        if (mode == 0)
            proc = pixel_span_draw_proc_X8R8G8B8_0;
        else if (mode == 1)
            proc = pixel_span_draw_proc_X8R8G8B8_1;
        else
            proc = pixel_span_draw_proc_X8R8G8B8_2;
        break;
    case R8G8B8:
        if (mode == 0)
            proc = pixel_span_draw_proc_R8G8B8_0;
        else if (mode == 1)
            proc = pixel_span_draw_proc_R8G8B8_1;
        else
            proc = pixel_span_draw_proc_R8G8B8_2;
        break;
    case R5G6B5:
        if (mode == 0)
            proc = pixel_span_draw_proc_R5G6B5_0;
        else if (mode == 1)
            proc = pixel_span_draw_proc_R5G6B5_1;
        else
            proc = pixel_span_draw_proc_R5G6B5_2;
        break;
    case A1R5G5B5:
        if (mode == 0)
            proc = pixel_span_draw_proc_A1R5G5B5_0;
        else if (mode == 1)
            proc = pixel_span_draw_proc_A1R5G5B5_1;
        else
            proc = pixel_span_draw_proc_A1R5G5B5_2;
        break;
    case A4R4G4B4:
        if (mode == 0)
            proc = pixel_span_draw_proc_A4R4G4B4_0;
        else if (mode == 1)
            proc = pixel_span_draw_proc_A4R4G4B4_1;
        else
            proc = pixel_span_draw_proc_A4R4G4B4_2;
        break;
    case X1R5G5B5:
        if (mode == 0)
            proc = pixel_span_draw_proc_X1R5G5B5_0;
        else if (mode == 1)
            proc = pixel_span_draw_proc_X1R5G5B5_1;
        else
            proc = pixel_span_draw_proc_X1R5G5B5_2;
        break;
    case G8:
        if (mode == 0)
            proc = pixel_span_draw_proc_G8_0;
        else if (mode == 1)
            proc = pixel_span_draw_proc_G8_1;
        else
            proc = pixel_span_draw_proc_G8_2;
        break;
    case UNKNOW:
        proc = NULL;
        break;
...
void BasicBitmap::SetDriver(PixelFmt fmt, BasicBitmap::PixelDraw proc,
    int mode)
{
    ...
    int w, int h, int mode, IUINT32 color)
{
    ...
    if (mode & PIXEL_FLAG_SRCOVER)
        op = 1;
    else if (mode & PIXEL_FLAG_ADDITIVE)
        op = 2;
...
// draw 8x16 glyph
//---------------------------------------------------------------------
void BasicBitmap::DrawGlyph(int x, int y, const void* glyph, int w, int h,
    IUINT32 color)
{
...
//---------------------------------------------------------------------
int BasicBitmap::SplitChannel(BasicBitmap* R, BasicBitmap* G, BasicBitmap* B,
    BasicBitmap* A)
{
...
// Search Palette
//---------------------------------------------------------------------
int BasicBitmap::BestfitColor(const BasicColor* pal, int r, int g, int b,
    int size)
{
...
void BasicBitmap::BresenhamStretch(int dx, int dy, int dw, int dh,
    const BasicBitmap* src, int sx, int sy,
    int sw, int sh, int mode)
{
    ...
    if ((mode & PIXEL_FLAG_NOCLIP) == 0) {
        ...
        if (dx < 0 || dx + dw > (int)_w || dy < 0 || dy + dh > (int)_h || sx < 0 || sx + sw > (int)src->_w || sy < 0 || sy + sh > (int)src->_h)
            return;
    }
    ...
    for (j = 0; j < dstheight; j++) {
        ...
        case 2: {
            ...
            if ((mode & PIXEL_FLAG_MASK) == 0) {
                for (i = dstwidth; i > 0; i--) {
                    *((unsigned short*)dstpix) = *((unsigned short*)srcpix);
                    dstpix += 2;
                    while (werr >= 0) {
                        srcpix += incx, werr -= dstwidth2;
                    }
...
void BasicBitmap::SetDriver(InterpRow fn) { InterpolateRowPtr = fn; }

void BasicBitmap::SetDriver(InterpCol fn) { InterpolateColPtr = fn; }
...
int BasicBitmap::InterpolateRowNearest(IUINT32* card, int w,
    const IUINT32* row1, const IUINT32* row2,
    IINT32 fraction)
{
    ...
    IUINT32 color)
{
    ...
    if (mode & PIXEL_FLAG_SRCOVER)
        draw = GetDriver(dfmt, 1, false);
    else if (mode & PIXEL_FLAG_ADDITIVE)
        draw = GetDriver(dfmt, 2, false);
    else if (mode & PIXEL_FLAG_SRCCOPY)
        draw = NULL;
    ...
    const BasicRect* bound, int mode, IUINT32 color)
{
    ...
    Scale(rd->left, rd->top, rd->right - rd->left, rd->bottom - rd->top, src,
        rs->left, rs->top, rs->right - rs->left, rs->bottom - rs->top, mode,
        color);
}
...
//---------------------------------------------------------------------
BasicBitmap* BasicBitmap::LoadBmpFromMemory(const void* buffer, long size,
    BasicColor* pal)
{
    ...
    if (pal != NULL && InfoHeader.biBitCount == 8) {
        for (IUINT32 i = 0; i < 256; i++) {
            if (ptr >= (const char*)buffer + offset)
                break;
...
// load bmp from file
//---------------------------------------------------------------------
BasicBitmap* BasicBitmap::LoadBmp(const char* filename, PixelFmt fmt,
    BasicColor* pal)
{
    void* content;
    long size;
...
static const char* PixelTgaReadn(const char* ptr, void* buffer, int w,
    int bpp)
{
    ...
    for (; w > 0; w--, lptr += n) {
        ...
        if (n == 1)
            _pixel_store_8(lptr, 0, c);
        else if (n == 2)
            _pixel_store_16(lptr, 0, c);
        else if (n == 3)
            _pixel_store_24(lptr, 0, c);
        else if (n == 4)
            _pixel_store_32(lptr, 0, c);
    }
    return ptr;
}

BasicBitmap* BasicBitmap::LoadTgaFromMemory(const void* buffer, long size,
    BasicColor* pal)
{
...
// load tga file
//---------------------------------------------------------------------
BasicBitmap* BasicBitmap::LoadTga(const char* filename, PixelFmt fmt,
    BasicColor* pal)
{
    void* content;
    long size;
...
// create dib section
void* BasicBitmap::CreateDIB(void* hDC, int w, int h, PixelFmt fmt,
    void** bits)
{
    ...
    int sw, int sh)
{
    ...
    if (rect.left < 0)
        x -= rect.left, rect.left = 0;
    if (rect.top < 0)
        y -= rect.top, rect.top = 0;
    if (rect.right > width)
        rect.right = width;
    ...
    hr = ::SetDIBitsToDevice((HDC)hDC, x, y, width, height, rect.left, rect.top,
        0, (int)this->_h, this->_bits, (LPBITMAPINFO)info,
        DIB_RGB_COLORS);
...
int BasicBitmap::GetDIBits(void* hDC, void* hBitmap)
{
    ...
    int hr = ::GetDIBits((HDC)hDC, (HBITMAP)hBitmap, 0, (int)this->_h,
        this->_bits, (LPBITMAPINFO)info, DIB_RGB_COLORS);
...
int BasicBitmap::GdiPlusInit(int startup)
{
    ...
    if (startup) {
        ...
        if (GdiPlusStartup_o == NULL) {
            GdiPlusStartup_o = (GdiPlusStartup_t)GetProcAddress(hDLL, "GdiplusStartup");
            if (GdiPlusStartup_o == NULL) {
                return -2;
            }
        }
        ...
        if (GdiPlusShutdown_o == NULL) {
            GdiPlusShutdown_o = (GdiPlusShutdown_t)GetProcAddress(hDLL, "GdiplusShutdown");
            if (GdiPlusShutdown_o == NULL) {
                return -4;
            }
        }
...
void* BasicBitmap::GdiPlusLoadCore(const void* data, long size, int* cx,
    int* cy, long* pitch, int* pfmt, int* bpp,
    int* errcode)
{
    ...
    if (CreateStream_o == NULL) {
        ...
        CreateStream_o = (CreateStream_t)GetProcAddress(hOleDLL, "CreateStreamOnHGlobal");
        if (CreateStream_o == NULL) {
            if (errcode)
                errcode[0] = -21;
            return NULL;
    ...
    if (GdipDisposeImage_o == NULL) {
        GdipDisposeImage_o = (GdipDisposeImage_t)GetProcAddress(hGdiPlusDLL, "GdipDisposeImage");
        if (GdipDisposeImage_o == NULL) {
            if (errcode)
                errcode[0] = -25;
            return NULL;
    ...
    if (GdipLockBits_o == NULL) {
        GdipLockBits_o = (GdipLockBits_t)GetProcAddress(hGdiPlusDLL, "GdipBitmapLockBits");
        if (GdipLockBits_o == NULL) {
            if (errcode)
                errcode[0] = -28;
            return NULL;
...
#define GPixelFormatMax 15
    ...
    if (gpixfmt == GPixelFormat8bppIndexed)
        gpixfmt = GPixelFormat8bppIndexed, fmt = 8, nbytes = 1;
    else if (gpixfmt == GPixelFormat16bppRGB555)
        gpixfmt = GPixelFormat16bppRGB555, fmt = 555, nbytes = 2;
    else if (gpixfmt == GPixelFormat16bppRGB565)
        gpixfmt = GPixelFormat16bppRGB565, fmt = 565, nbytes = 2;
    else if (gpixfmt == GPixelFormat16bppARGB1555)
        gpixfmt = GPixelFormat16bppARGB1555, fmt = 1555, nbytes = 2;
    else if (gpixfmt == GPixelFormat24bppRGB)
        gpixfmt = GPixelFormat24bppRGB, fmt = 888, nbytes = 3;
    else if (gpixfmt == GPixelFormat32bppRGB)
        gpixfmt = GPixelFormat32bppRGB, fmt = 888, nbytes = 4;
    else if (gpixfmt == GPixelFormat32bppARGB)
        gpixfmt = GPixelFormat32bppARGB, fmt = 8888, nbytes = 4;
...
BasicBitmap* BasicBitmap::GdiPlusLoadImageFromMemory(const void* data,
    long size, int* err)
{
    void* bits = NULL;
    int cx = -1;
...
bool BasicBitmap::AlphaBlend(void* hdcDst, int nXOriginDest, int nYOriginDest,
    int nWidthDest, int nHeightDest, void* hdcSrc,
    int nXOriginSrc, int nYOriginSrc, int nWidthSrc,
    int nHeightSrc, int SCA)
{
    typedef BOOL(WINAPI * AlphaBlend_t)(HDC, int, int, int, int, HDC, int, int,
        int, int, DWORD);
    ...
    BOOL hr = AlphaBlend_o((HDC)hdcDst, nXOriginDest, nYOriginDest, nWidthDest,
        nHeightDest, (HDC)hdcSrc, nXOriginSrc, nYOriginSrc,
        nWidthSrc, nHeightSrc, blend);
    return (hr) ? true : false;
}

// call TransparentBlt from msimg32.dll
bool BasicBitmap::TransparentBlt(void* hdcDst, int nXOriginDest,
    int nYOriginDest, int nWidthDest,
    int nHeightDest, void* hdcSrc, int nXOriginSrc,
    int nYOriginSrc, int nWidthSrc, int nHeightSrc,
    IUINT32 crTransparent)
{
    typedef BOOL(WINAPI * TransparentBlt_t)(HDC, int, int, int, int, HDC, int,
        int, int, int, UINT);
    ...
    if (TransparentBlt_o == NULL) {
        ...
        if (hDLL == NULL)
            return false;
        TransparentBlt_o = (TransparentBlt_t)GetProcAddress(hDLL, "TransparentBlt");
        if (TransparentBlt_o == NULL)
            return false;
    }
    BOOL hr = TransparentBlt_o((HDC)hdcDst, nXOriginDest, nYOriginDest, nWidthDest,
        nHeightDest, (HDC)hdcSrc, nXOriginSrc, nYOriginSrc,
        nWidthSrc, nHeightSrc, (UINT)crTransparent);
    return (hr) ? true : false;
}

// CreateDIBSection and then create BasicBitmap in the DIB (borror pixel memory)
// DeleteObject((HBITMAP)hbitmap) must be invoked to dispose DIB after bitmap
// destruction
BasicBitmap* BasicBitmap::CreateBitmapInDIB(void* hDC, int w, int h,
    PixelFmt fmt, void** hbitmap)
{
...
//---------------------------------------------------------------------
BasicBitmap* BasicBitmap::LoadImageFromMemory(const void* data, long size,
    BasicColor* pal)
{
...
// detect file type and load
//---------------------------------------------------------------------
BasicBitmap* BasicBitmap::LoadFile(const char* filename, PixelFmt fmt,
    BasicColor* pal)
{
    void* content;
    long size;
...
//---------------------------------------------------------------------
int BasicBitmap::DownSampleBy2(int x, int y, const BasicBitmap* src, int sx,
    int sy, int sw, int sh)
{
    ...
    if (Bpp() == 32) {
        for (int j = 0; j < h; j++) {
            ...
            for (int i = w; i > 0; row1 += 2, row2 += 2, dst++, i--) {
                IUINT32 rb = (row1[0] & 0xff00ff) + (row1[1] & 0xff00ff) + (row2[0] & 0xff00ff) + (row2[1] & 0xff00ff);
                IUINT32 ag = ((row1[0] >> 8) & 0xff00ff) + ((row1[1] >> 8) & 0xff00ff) + ((row2[0] >> 8) & 0xff00ff) + ((row2[1] >> 8) & 0xff00ff);
                rb = (rb >> 2) & 0xff00ff;
                ag = (ag >> 2) & 0xff00ff;
                dst[0] = rb | (ag << 8);
            }
    ...
    if (Bpp() == 24) {
        for (int j = 0; j < h; j++) {
            ...
            for (int i = w; i > 0; row1 += 6, row2 += 6, dst += 3, i--) {
                ...
                IUINT32 rb = (c1 & 0xff00ff) + (c2 & 0xff00ff) + (c3 & 0xff00ff) + (c4 & 0xff00ff);
                IUINT32 ag = ((c1 >> 8) & 0xff) + ((c2 >> 8) & 0xff) + ((c3 >> 8) & 0xff) + ((c4 >> 8) & 0xff);
                rb = (rb >> 2) & 0xff00ff;
                ag = (ag >> 2) & 0x0000ff;
                c1 = rb | (ag << 8);
                _pixel_write_24(dst, c1);
    ...
    if (Bpp() == 16) {
        for (int j = 0; j < h; j++) {
            ...
            case A4R4G4B4:
                for (i = w; i > 0; row1 += 2, row2 += 2, dst++, i--) {
                    c1 = (row1[0] & 0x0f0f) + (row1[1] & 0x0f0f) + (row2[0] & 0x0f0f) + (row2[1] & 0x0f0f);
                    c2 = ((row1[0] >> 4) & 0x0f0f) + ((row1[1] >> 4) & 0x0f0f) + ((row2[0] >> 4) & 0x0f0f) + ((row2[1] >> 4) & 0x0f0f);
                    c3 = (c1 >> 2) & 0x0f0f;
                    c4 = (c2 >> 2) & 0x0f0f;
...
// BilinearSampler
//---------------------------------------------------------------------
static inline IUINT32 _pixel_biline_interp(IUINT32 tl, IUINT32 tr, IUINT32 bl,
    IUINT32 br, IINT32 distx,
    IINT32 disty)
{
...
inline int _pixel_abs(int x) { return (x < 0) ? (-x) : x; }
...
static inline float _pixel_bicubic(float x)
{
    ...
    float x3 = x2 * x;
    if (x <= 1.0f)
        return x3 * (a + 2.0f) - x2 * (a + 3.0f) + 1.0f;
    if (x <= 2.0f)
        return x3 * a - x2 * a * 5.0f + 8.0f * a * x - 4.0f * a;
    return 0.0f;
}
...
//---------------------------------------------------------------------
BasicBitmap* BasicBitmap::Resample(int NewWidth, int NewHeight,
    ResampleFilter filter) const
{
...
typedef int (*BasicBitmap_ResampleShrinkX)(IUINT8* dstpix, const IUINT8* srcpix,
    int height, long dstpitch,
    long srcpitch, int dstwidth,
    int srcwidth, void* workmem);

// shrink y type
typedef int (*BasicBitmap_ResampleShrinkY)(IUINT8* dstpix, const IUINT8* srcpix,
    int width, long dstpitch,
    long srcpitch, int dstheight,
    int srcheight, void* workmem);
    ...
    int srcheight, void* workmem)
{
    ...
    for (y = 0; y < srcheight; y++) {
        ...
        if (ycounter > 0x10000) {
            ...
            IINT32 yr = yrecip;
            for (x = 0; x < width;
                 dstpix += 4, srcpix += 4, accumulate += 4, x++) {
                dstpix[0] = (IUINT8)(
                    ((accumulate[0] + ((srcpix[0] * yc) >> 16)) * yr) >> 16);
                dstpix[1] = (IUINT8)(
                    ((accumulate[1] + ((srcpix[1] * yc) >> 16)) * yr) >> 16);
                dstpix[2] = (IUINT8)(
                    ((accumulate[2] + ((srcpix[2] * yc) >> 16)) * yr) >> 16);
                dstpix[3] = (IUINT8)(
                    ((accumulate[3] + ((srcpix[3] * yc) >> 16)) * yr) >> 16);
            }
            dstpix += dstdiff;
            accumulate = templine;
            srcpix -= 4 * width;
...
// resample drivers
//---------------------------------------------------------------------
static BasicBitmap_ResampleShrinkX BasicBitmap_ResampleShrinkX_Ptr = BasicBitmap_ResampleShrinkX_C;

static BasicBitmap_ResampleShrinkY BasicBitmap_ResampleShrinkY_Ptr = BasicBitmap_ResampleShrinkY_C;

static BasicBitmap_ResampleExpandX BasicBitmap_ResampleExpandX_Ptr = BasicBitmap_ResampleExpandX_C;

static BasicBitmap_ResampleExpandY BasicBitmap_ResampleExpandY_Ptr = BasicBitmap_ResampleExpandY_C;
...
void BasicBitmap_ResampleDriver(int id, void* ptr)
{
    switch (id) {
    case 0:
        BasicBitmap_ResampleShrinkX_Ptr = (ptr == NULL) ? BasicBitmap_ResampleShrinkX_C
                                                        : (BasicBitmap_ResampleShrinkX)ptr;
        break;
    case 1:
        BasicBitmap_ResampleShrinkY_Ptr = (ptr == NULL) ? BasicBitmap_ResampleShrinkY_C
                                                        : (BasicBitmap_ResampleShrinkY)ptr;
        break;
    case 3:
        BasicBitmap_ResampleExpandX_Ptr = (ptr == NULL) ? BasicBitmap_ResampleExpandX_C
                                                        : (BasicBitmap_ResampleExpandX)ptr;
        break;
    case 4:
        BasicBitmap_ResampleExpandY_Ptr = (ptr == NULL) ? BasicBitmap_ResampleExpandY_C
...
// ResampleSmooth
//---------------------------------------------------------------------
int BasicBitmap_ResampleSmooth(IUINT8* dstpix, const IUINT8* srcpix,
    int dstwidth, int srcwidth, int dstheight,
    int srcheight, long dstpitch, long srcpitch)
{
    ...
    if (dstwidth == srcwidth) {
        if (dstheight < srcheight) {
            if (BasicBitmap_ResampleShrinkY_Ptr(dstpix, srcpix, srcwidth,
                    dstpitch, srcpitch, dstheight,
                    srcheight, workmem)
                != 0) {
                delete[] temp;
                return -2;
            ...
            if (BasicBitmap_ResampleExpandY_Ptr(dstpix, srcpix, srcwidth,
                    dstpitch, srcpitch, dstheight,
                    srcheight, workmem)
                != 0) {
                delete[] temp;
    ...
    if (dstheight < srcheight) {
        if (BasicBitmap_ResampleShrinkY_Ptr(temp, srcpix, srcwidth,
                srcwidth * 4, srcpitch, dstheight,
                srcheight, workmem)
            != 0) {
            delete[] temp;
            return -4;
        ...
        if (BasicBitmap_ResampleExpandY_Ptr(temp, srcpix, srcwidth,
                srcwidth * 4, srcpitch, dstheight,
                srcheight, workmem)
            != 0) {
            delete[] temp;
        ...
        if (dstwidth < srcwidth) {
            if (BasicBitmap_ResampleShrinkX_Ptr(dstpix, srcpix, dstheight,
                    dstpitch, srcpitch, dstwidth,
                    srcwidth, workmem)
                != 0) {
                delete[] temp;
                return -6;
            ...
            if (BasicBitmap_ResampleExpandX_Ptr(dstpix, srcpix, dstheight,
                    dstpitch, srcpitch, dstwidth,
                    srcwidth, workmem)
                != 0) {
                delete[] temp;
...
int BasicBitmap_ResampleAvg(BasicBitmap* dst, int dx, int dy, int dw, int dh,
    const BasicBitmap* src, int sx, int sy, int sw,
    int sh)
{
    ...
    if (src->Bpp() == 32) {
        ...
        if (bmp == NULL)
            return -1;
        int hr = BasicBitmap_ResampleSmooth(
            bmp->Address8(0, 0), (const IUINT8*)src->Address32(sx, sy),
            dstwidth, srcwidth, dstheight, srcheight, bmp->Pitch(),
            src->Pitch());
        if (hr != 0) {
            delete bmp;
            return -2;
    ...
    if (dst->Bpp() == 32) {
        ...
        int hr = BasicBitmap_ResampleSmooth(
            (IUINT8*)dst->Address32(dx, dy),
            (const IUINT8*)bmp->Address32(0, 0), dstwidth, srcwidth, dstheight,
            srcheight, dst->Pitch(), bmp->Pitch());
        delete bmp;
        if (hr != 0) {
            return -4;
        }
    ...
    int hr = BasicBitmap_ResampleSmooth(bs->Address8(0, 0), bd->Address8(0, 0),
        dstwidth, srcwidth, dstheight,
        srcheight, bd->Pitch(), bs->Pitch());
...
// Resample
//---------------------------------------------------------------------
int BasicBitmap::Resample(int dx, int dy, int dw, int dh,
    const BasicBitmap* src, int sx, int sy, int sw,
    int sh, ResampleFilter filter)
{
    if ((_w | _h | src->_w | src->_h) >= 0x7fff)
        return -1;
    ...
    if (dx < 0 || dx + dw > (int)_w || dy < 0 || dy + dh > (int)_h || sx < 0 || sx + sw > (int)src->_w || sy < 0 || sy + sh > (int)src->_h || dh <= 0 || dw <= 0)
        return 0;
    ...
    case NONE:
        ...
        break;
    case LINEAR:
        Scale(dx, dy, dw, dh, src, sx, sy, sw, sh, mode | PIXEL_FLAG_LINEAR,
            0xffffffff);
        break;
    case BILINEAR:
        Scale(dx, dy, dw, dh, src, sx, sy, sw, sh,
            mode | PIXEL_FLAG_BILINEAR, 0xffffffff);
        break;
    case AVERAGE:
